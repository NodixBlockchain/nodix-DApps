             ...
             png_charp profile, png_uint_32 proflen)
{
   ...
   png_strcpy(new_iccp_name,png_strlen(name)+1, name);
             ...
             png_infop info_ptr, png_sPLT_tp entries, int nentries)
{
    ...
    for (i = 0; i < nentries; i++)
    {
        ...
        png_strcpy(to->name,png_strlen(from->name) + 1, from->name);
   ...
   png_infop info_ptr, png_unknown_chunkp unknowns, int num_unknowns)
{
    ...
    np = (png_unknown_chunkp)png_malloc_warn(png_ptr, (info_ptr->unknown_chunks_num + num_unknowns) *    png_sizeof(png_unknown_chunk));
    ...
    for (i = 0; i < num_unknowns; i++)
    {
		...
		to->name[1]=from->name[1];
		to->name[2]=from->name[2];
		to->name[3]=from->name[3];
		to->name[4]=from->name[4];
		//to->name[5]=0;
