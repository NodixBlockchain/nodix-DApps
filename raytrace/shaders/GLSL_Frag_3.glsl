...
uniform	highp  mat3	cameraInvNormal;
uniform	highp  mat3	cameraInvMat;
uniform	highp  vec3	cameraPos;

uniform	highp  float	winW;
uniform	highp  float	winH;
...
uniform highp  int num_materials;

//axis aligned bounding box
uniform highp  int numboxes;

//cubes
uniform highp  int num_cubes;

//sphere
uniform highp  int num_spheres;

//cylinder
uniform highp  int num_cyls;

//planes
uniform highp  int num_planes;

uniform	highp  float		PI;
uniform	highp  float		invRes;
uniform	highp  float		D3_EPSILON;
...
int Sphere_intersect(int sp_idx, vec3 raypos, vec3 direction, float minDist, out float d0, out vec3 sphere_norm, out vec2 sphere_uv)
{
	...
	sphere_uv[1]	=	asin(sphere_norm[2])/(PI) + 0.5;
		...
		if (CastId!=item_ID)
		{
			if(intersect_ray_box(i,ray_origin,ray_inv_direction)==1)
			{
				if (item_type == 1)
				{
					...
					intersect = Sphere_intersect(item_idx, ray_origin, ray_direction, distFin, sphere_dist1, sphere_norm, sphere_uv);
				}
				else if (item_type == 3)
				{
					vec3		cyl_norm;
		...
		if(inCastId!=item_ID)
		{
			if(intersect_ray_box(i,ray_origin,ray_inv_direction)==1)
			{
				...
				if (item_type == 1)
				{
					...
					if (Sphere_intersect(item_idx, ray_origin, ray_direction, dist_final, sphere_dist1, sphere_norm, sphere_uv) == 1)
					{
...
void main()
{
	...
	if (castRay(ray_origin, ray_direction, ray_inv_direction, pix, outPos, normVec, matId, origId) == 1)
	{
		...
		if (refract_fac > 0.0)
		{
			...
			pix = pix*(1.0 - refract_fac)+refrPix*refract_fac;
		}
